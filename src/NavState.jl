...
"""
function NavState(t, x, P, ns=size(P, 1); type::Symbol=:EKF, α=1e-3, β=2.0, κ=0.0, iter=5) :: AbstractNavState
    if type == :SRUKF
    ...
    elseif type == :IEKF
        return NavStateEKF(t, copy(x), copy(P), ns; iter=iter)
    end
    return NavStateEKF(t, copy(x), copy(P), ns; iter=0)
end
